...
/**
 * URL prefix to download first content from site
 */
var urlPrefix = 'http://688e3a52.ngrok.io/query_fields.json';
...
var steps = [
    ...
    "Brush with more oil mixture", "sprinkle with seeds",
    "Cut each into 2 triangles",
    "Bake until golden, about 13 minutes",
    "Whisk everything", "Drizzle on chicken", "let cool"];

/**
 * LetsCookFood is a child of AlexaSkill.
 * To read more about inheritance in JavaScript, see the link below.
 *
 * @see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Introduction_to_Object-Oriented_JavaScript#Inheritance
 */
var LetsCookFood = function() {
    AlexaSkill.call(this, APP_ID);
};

// Extend AlexaSkill
LetsCookFood.prototype = Object.create(AlexaSkill.prototype);
LetsCookFood.prototype.constructor = LetsCookFood;

LetsCookFood.prototype.eventHandlers.onSessionStarted = function (sessionStartedRequest, session) {
    console.log("LetsCookFood onSessionStarted requestId: " + sessionStartedRequest.requestId
        + ", sessionId: " + session.sessionId);

    // any session init logic would go here
};

LetsCookFood.prototype.eventHandlers.onLaunch = function (launchRequest, session, response) {
    console.log("LetsCookFood onLaunch requestId: " + launchRequest.requestId + ", sessionId: " + session.sessionId);
    getWelcomeResponse(response);
};

LetsCookFood.prototype.eventHandlers.onSessionEnded = function (sessionEndedRequest, session) {
...
LetsCookFood.prototype.intentHandlers = {
    ...
    "RecipeIntent": function(intent, session, response) {
        recipe(intent, session, response);
    },

    "PantryIntent": function (intent, session, response) {
        pantry(intent, session, response);
    },
    "AMAZON.HelpIntent": function (intent, session, response) {
        var speechText = "With Let's Cook Food, you can organize your pantry of ingredients, search and favorite recipes, keep track of your shopping list, or cook a recipe.  " +
            "For example, you could find an amazing sesame chicken recipe, add the necessary ingredients to your shopping list, and cook the recipe when ready.";
...
function getWelcomeResponse(response) {
    ...
    var cardTitle = "Let's Cook Some Food";
    var repromptText = "With Let's Cook Food, you can organize your pantry of ingredients, search and favorite recipes, keep track of your shopping list, or cook a recipe. Now, which would you like to do?";
    var speechText = "<p>Home Cook</p> <p>What would you like to do?</p>";
    var cardOutput = "Home Cook, what would you like to do?";
...
function handleFirstEventRequest(intent, session, response) {
    ...
    // Read the first 10 events, then set the count to 10
    sessionAttributes.index = paginationSize;
    var date = "";


    if (daySlot == "Recipe") {
        var speechText = "Alright Let's cook some Sesame Chicken!";
    ...
    getJsonEventsFromWikipedia(monthNames[date.getMonth()], date.getDate(), function (events) {
        ...
        if (events.length == 0) {
            speechText = "There is a problem connecting to Let's Cook Food. You're going hungry. Please don't die.";
            cardContent = speechText;
            response.tell(speechText);
        } else {
...
function pantry(intent, session, response) {

    var cardTitle = "Let's Cook Some Food";
    var repromptText = "With Let's Cook Food, you can organize your pantry of ingredients, search and favorite recipes, keep track of your shopping list, or cook a recipe. Now, which would you like to do?";
    var speechText = "<p>Welcome to the pantry</p> <p>What would you like to do?</p>";
    var cardOutput = "Home Cook, what would you like to do?";
    // If the user either does not reply to the welcome message or says something that is not
    // understood, they will be prompted again with this text.
...
function recipe(intent, session, response) {

    var cardTitle = "Let's Cook Some Food";
    var repromptText = "Are you ready for your next step?";

    if (index == 0) {
        var speechText = "<p>Alright</p> <p>Let's cook some sesame chicken!</p>";
    }
    else {
        var speechText = "";
    }
    ...
    if (index < steps.length -1) {
        speechText = speechText + steps[index];
        index = index + 1;


    }
    else {
        speechText = "Looks like we're done. Enjoy!";
    };
...
function handleNextEventRequest(intent, session, response) {
    var cardTitle = "More events on this day in history",
        ...
        i;
    if (!result) {
        speechText = "With Let's Cook Food you can cook Food.";
        cardContent = speechText;
